...
// ---------------------------------------------------------
// 1. BASIS DATA MATERI, IKTP, GAME, & LKM PER TUJUAN PEMBELAJARAN
// ---------------------------------------------------------
const DATABASE_TP = [
  {
    id: 0,
    ...
    materi: `
      ...
      <div style="text-align:center; padding: 12px; background:#f0f9ff; border-radius:8px; margin: 12px 0; border: 1.5px solid #bae6fd;">
        <span style="font-size:1.3rem; font-weight:700; color:#0284c7;">Rumus: ∑F = 0</span><br>
        <small style="color:#64748b;">(Dibaca: Total penjumlahan seluruh gaya sama dengan nol)</small>
      </div>
      ...
      <div style="text-align:center; padding: 14px; background:#f0f9ff; border-radius:8px; margin: 12px 0; border: 1.5px solid #bae6fd;">
        <span style="font-size:1.4rem; font-weight:700; color:#0284c7;">F = m × a &nbsp; atau &nbsp; a = F / m</span><br>
        <p style="margin-top:6px; font-size:0.88rem; color:#334155;">
      ...
      <div style="text-align:center; padding: 12px; background:#f0f9ff; border-radius:8px; margin: 12px 0; border: 1.5px solid #bae6fd;">
        <span style="font-size:1.3rem; font-weight:700; color:#0284c7;">F_aksi = - F_reaksi</span><br>
        <small style="color:#64748b;">(Tanda minus artinya arah gayanya berlawanan arah)</small>
      </div>
      ...
      <div class="misconception-box">
        ...
        Banyak murid mengira benda yang sedang diam itu sama sekali "tidak memiliki gaya". Itu salah! Benda diam (misalnya buku di atas meja) tetap ditarik oleh gaya berat gravitasi bumi ke bawah, tetapi ditahan oleh gaya normal meja ke atas. Karena gayanya seimbang (∑F = 0), maka buku tersebut tetap diam tenang di meja.
      </div>
    `,
  ...
  },
  {
    ...
    materi: `
      ...
      <div style="text-align:center; padding: 12px; background:#f0f9ff; border-radius:8px; margin: 12px 0; border: 1.5px solid #bae6fd;">
        <span style="font-size:1.3rem; font-weight:700; color:#0284c7;">Rumus: P = ρ × g × h</span><br>
        <p style="margin-top:6px; font-size:0.86rem; color:#334155;">
      ...
      <div style="text-align:center; padding: 12px; background:#f0f9ff; border-radius:8px; margin: 12px 0; border: 1.5px solid #bae6fd;">
        <span style="font-size:1.3rem; font-weight:700; color:#0284c7;">Rumus: F₁ / A₁ = F₂ / A₂</span><br>
        <small style="color:#64748b;">F = Gaya tekan (Newton), A = Luas penampang pipa (m²)</small>
      </div>
  ...
  },
  {
    ...
    materi: `
      ...
      <div style="text-align:center; padding: 12px; background:#f0f9ff; border-radius:8px; margin: 12px 0; border: 1.5px solid #bae6fd;">
        <span style="font-size:1.3rem; font-weight:700; color:#0284c7;">Rumus: ΔU = Q - W</span><br>
        <p style="margin-top:6px; font-size:0.86rem; color:#334155;">
  ...
  },
  {
    ...
    materi: `
      ...
      <div style="text-align:center; padding: 12px; background:#f0f9ff; border-radius:8px; margin: 12px 0; border: 1.5px solid #bae6fd;">
        <span style="font-size:1.4rem; font-weight:700; color:#0284c7;">v = λ × f</span><br>
        <p style="margin-top:6px; font-size:0.86rem; color:#334155;">
...
// ---------------------------------------------------------
// 2. ARSIP MODUL AJAR / RPP GURU KURIKULUM MERDEKA
// ---------------------------------------------------------
...
// ---------------------------------------------------------
// 3. DAFTAR LENGKAP 35 SISWA KELAS XI
// ---------------------------------------------------------
const DAFTAR_SISWA = [
...
// ---------------------------------------------------------
// 5. AUTENTIKASI & MANAJEMEN SESI LOGIN
// ---------------------------------------------------------
...
// ---------------------------------------------------------
// 7. LOGIKA DASBOR GURU / ADMIN (7 SUB-TAB)
// ---------------------------------------------------------
...
function bukaModalDetailJurnal(id) {
  ...
  document.getElementById("modal-detail-body").innerHTML = `
    <div style="margin-bottom:14px; background:#f0f9ff; padding:14px; border-radius:8px; border:1.5px solid #bae6fd;">
      <strong style="color:#0369a1;">Aktivitas Pembelajaran Mendalam (Deep Learning):</strong>
...
function bukaModalDetailModul(idx) {
  ...
  document.getElementById("modal-detail-body").innerHTML = `
    ...
    <div style="margin-bottom:12px; background:#f0f9ff; padding:12px; border-radius:6px; border:1px solid #bae6fd;">
      <strong style="color:#0369a1;">Deskripsi & Pemahaman Bermakna:</strong>
    ...
    <div style="margin-bottom:14px; background:#ecfdf5; padding:12px; border-radius:6px; border:1px solid #a7f3d0;">
      <strong style="color:#065f46;">Alur Sintaks Pembelajaran Mendalam (Deep Learning):</strong>
...
// ---------------------------------------------------------
// 9. DATA SIMULASI UJI COBA CEPAT (DUMMY DATA)
// ---------------------------------------------------------
function isiContohDataSimulasi() {
  ...
  localStorage.setItem("lkm_TP 1.1_Abel Pratama Katili", JSON.stringify({
    ...
    q1: "Saat beban digandakan dari 50 kg ke 100 kg dengan gaya dorong tetap 100 N, percepatan kotak berkurang menjadi setengahnya (dari 2 m/s² menjadi 1 m/s²).",
    q2: "Lantai bergesek kasar menahan kotak bergerak sampai gaya dorong melebihi gaya gesek statis maksimum.",
    q3: "1. Penumpang terdorong ke depan saat bus direm (Hukum I). 2. Saat mendayung perahu, dayung mendorong air ke belakang dan perahu meluncur maju (Hukum III).",
    waktu: new Date().toLocaleString()
  }));

  // Simulasi Peta Konsep
  localStorage.setItem("peta_TP 1.1_Dimas Saputra R. Antu", JSON.stringify({
    siswa: "Dimas Saputra R. Antu", tp: "TP 1.1",
    utama: "Hukum-Hukum Newton tentang Gerak",
    cabang: "• Hukum I (Inersia) -> Sabuk pengaman mobil\n• Hukum II (F = m x a) -> Mendorong gerobak berat\n• Hukum III (Aksi-Reaksi) -> Menjejak kaki saat jalan",
    kesulitan: "Membedakan mana gaya aksi dan gaya reaksi saat tali menarik benda.",
    gambar: "",
    waktu: new Date().toLocaleString()
  }));

  // Simulasi Penilaian Diri
  localStorage.setItem("eval_diri_Adeliani Putri R. Agu", JSON.stringify({
    siswa: "Adeliani Putri R. Agu",
    dipahami: "Saya sudah sangat memahami konsep inersia Hukum I Newton dan mengapa kita wajib menggunakan helm dan sabuk pengaman saat berkendara.",
    belumDipahami: "Perhitungan matematis kombinasi gaya gesek statis dan kinetis saat benda mulai meluncur.",
    rencanaTindakan: "Saya akan mengulang simulasi PhET dan berdiskusi dengan teman sekelompok.",
    waktu: new Date().toLocaleString()
  }));
  ...
  localStorage.setItem("eval_peer_Moh. Riski Ahmad_oleh_Andika Pratama Latoini", JSON.stringify({
    ...
    tp: "TP 1.1",
    skor: { penguasaanKonsep: "4", investigasiLab: "4", kolaborasiDiskusi: "4", sikapSolutif: "4" },
    catatan: "Riski sangat aktif dan cermat saat memanipulasi variabel pada simulasi gerak PhET.",
    waktu: new Date().toLocaleString()
  }));

  alert("✓ Data simulasi (Presensi, Jurnal, LKM, Peta Konsep, dan Penilaian) berhasil dimasukkan!");
  renderTabelGuruPresensi();
  renderTabelGuruJurnal();
  renderTabelGuruLKM();
  renderTabelGuruPeta();
